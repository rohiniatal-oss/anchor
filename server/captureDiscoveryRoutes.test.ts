...
test("capture suggest labels search discovery as automatic ranked preview work", async () => {
  ...
  assert.match(response.json.suggestion.reason, /rank the results/i);
});
...
test("automatic discovery returns evidence ranked options and a preview without creating objects", async () => {
  ...
  assert.ok(response.json.rankedOptions.length >= 1);
  assert.equal(response.json.rankedOptions[0].rank, 1);
  assert.equal(response.json.rankedOptions[0].kind, "role");
  assert.match(response.json.rankedOptions[0].nextAction, /verify.*current opportunity/i);
  assert.match(response.json.recommendedNextAction, /create a Job only if/i);
