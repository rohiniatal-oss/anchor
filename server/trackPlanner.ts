...
export function buildTrackPlan(track: CareerTrack, data: { tasks: Task[]; jobs: Job[]; learn: Learn[]; hustles: Hustle[]; contacts: Contact[] }): TrackPlan {
  ...
  if (jobs.length < 3) needs.push({ lane: "Direction", priority: 100, stage: "signal", kind: "anchor", move: `Review three real ${track.name} roles and note the requirements that keep coming up`, doneWhen: "Three role examples are saved with the main requirements they share", reason: "The track does not yet have enough real role evidence." });
  ...
  if (learn.length > 0 && !learn.some((l) => l.outputEvidenceUrl || l.done)) needs.push({ lane: "Learning", priority: 48, stage: "convert", kind: "ongoing", move: `Turn one ${track.name} learning item into a useful note`, doneWhen: "One learning item has notes or a prep note you can use again", reason: "Learning should compound into track leverage, but not crowd out applications." });
