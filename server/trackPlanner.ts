...
export function buildTrackPlan(track: CareerTrack, data: { tasks: Task[]; jobs: Job[]; learn: Learn[]; hustles: Hustle[]; contacts: Contact[] }): TrackPlan {
  ...
  if (jobs.length < 3) needs.push({ lane: "Direction", priority: 100, stage: "signal", kind: "anchor", move: `Find one real ${track.name} role and extract the first requirements pattern`, doneWhen: "One real role and one repeated requirements pattern are captured", reason: "The track does not yet have enough real role evidence." });
