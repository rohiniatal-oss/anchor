...
function uniqueStrings(values: unknown[]): string[] {
  ...
  const result: string[] = [];
  for (const value of values.map((item) => compact(item, 300)).filter(Boolean)) {
...
function parseSteps(value: unknown): Array<{ text: string; done: boolean; condition?: string; executor?: string }> {
  try {
...
function evidenceSummary(task: Task, blueprintTask: TaskBlueprint): string {
  ...
  return compact([
    ...
    task.sourceUrl ? `Evidence link: ${task.sourceUrl}.` : "",
  ].filter(Boolean).join(" "), 4_000);
}

function questionForKind(kind: TaskBlueprintKind, expectedEvidence: string): string {
  if (kind === "relationship") return "What concrete outcome came from this relationship action?";
  if (kind === "access") return "What access signal did this action create?";
  if (kind === "experience") return "Did this applied work produce a concrete outcome you can describe?";
  if (kind === "credential") return "Was the formal requirement completed, or did this only move it forward?";
  ...
  return "Did completing this task create the expected evidence?";
}
...
function optionsForKind(kind: TaskBlueprintKind): ExecutionOutcomeOption[] {
  const evidenceLabel = kind === "relationship"
    ...
    : kind === "access"
      ...
      : kind === "credential"
        ? "The credential or formal evidence was completed"
        : kind === "artifact"
          ? "The artifact exists and is ready to inspect"
          : "The work produced a concrete outcome I can describe";
  const partialLabel = kind === "credential"
    ...
    : kind === "artifact"
      ...
      : "The action was completed and produced a limited signal";
  return [
    {
      id: "evidence_created",
      label: evidenceLabel,
    ...
    {
      id: "no_evidence",
      label: "The action was completed, but it did not create useful evidence",
    ...
    },
    {
      ...
      description: "Anchor will reopen the task and will not create an evidence record.",
      strength: "none",
      usableForCoverage: false,
      reopensTask: true,
    },
...
export function normalizeExecutionOutcomeModel(
  ...
  const outcomes = Array.isArray(raw.outcomes)
    ? raw.outcomes.filter((outcome) => outcome && typeof outcome === "object" && Number(outcome.trackId) === trackId)
    : [];
  ...
  return {
    ...
    trackId,
    currentBlueprintFingerprint,
    outcomes: outcomes as ExecutionOutcome[],
    pendingOutcomeIds,
    queuedOutcomeIds,
    generatedAt: Date.now(),
  };
  ...
  return {
    ...
    strength: outcome.strength === "verified"
      ...
      : outcome.strength === "direct"
        ? "direct"
        : "supporting",
    state: outcome.evidenceUrl ? "published" : outcome.taskKind === "relationship" ? "active" : "completed",
    usableForCoverage: true,
