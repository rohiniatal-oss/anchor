...
const PROJECT_SIGNAL_RE = /\b(strategy|transition|search|campaign|launch|programme|program|portfolio|pipeline|path|move into|prepare for|build toward|set up|end to end|from scratch)\b/i;
const BOUNDED_OUTPUT_RE = /\b(one|1|single|short|five[- ]line|one[- ]page|email|message|note|brief|list|outline|draft|answer|checklist|decision|comparison|three|3)\b/i;
...
function classifyIntent(title: string) {
  ...
  if (/^(?:write|draft|create|build|develop|design|outline|launch)\b/i.test(title)) return "creation";
  if (/^(?:improve|fix|strengthen|refine|revise|update)\b/i.test(title)) return "improvement";
  if (/^(?:organize|organise|sort\s+out|plan|work\s+on|set\s+up)\b/i.test(title)) return "organization";
  return "general";
}
...
function inferScope(title: string, intent: string): WorkScope {
  ...
  if (PROJECT_SIGNAL_RE.test(title) || hasMultiOutcomeLanguage(title)) return /\b(month|quarter|ongoing|transition|campaign|programme|program)\b/i.test(title) ? "multi_week" : "multi_session";
  if (["research", "preparation", "creation", "improvement", "organization"].includes(intent) && !BOUNDED_OUTPUT_RE.test(title)) return "multi_session";
...
function definitionTemplate(input: {
  workType: WorkType;
  intent: string;
  target: string;
...
export function interpretWorkDeterministically(input: WorkInterpretationInput): WorkDefinition {
  ...
  const template = definitionTemplate({ workType, intent, target, resolvedTarget: resolution.value, purpose, whyNow, scope } as any);
